  ...
  return (
    <section className="px-6 md:px-10 lg:px-12 py-16 bg-white font-urbanist">
      ...
      <div className="flex items-end justify-between mb-12 border-b border-gray-100 pb-8">
        <div className="flex items-center gap-6">
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              <span className="text-[10px] font-bold tracking-[0.3em] uppercase text-orange-600">Limited Time</span>
              <span className="h-1 w-1 rounded-full bg-gray-300"></span>
              <span className="text-[10px] font-bold tracking-[0.1em] uppercase text-slate-400">Ends in: 04:22:15</span>
            </div>
        ...
        <Link to="/deals" className="text-xs font-bold uppercase tracking-widest text-slate-900 flex items-center gap-2 hover:text-blue-600 transition-colors pb-1">
          ...
          <Link to={`/product/${deal.id}`} key={deal.id}>
            ...
            >
              <div className="w-1/2 p-8 lg:p-12">
                <span className="inline-block py-1.5 px-4 rounded-full bg-orange-500 text-white text-[10px] font-bold uppercase tracking-widest mb-6 shadow-lg shadow-orange-500/20">
                  Save ${(deal.price * 0.2).toFixed(0)}
                </span>
                <h3 className="text-2xl lg:text-3xl font-bold text-slate-900 er mb-4 uppercase line-clamp-2">{deal.name}</h3>
                              <div className="flex items-center gap-4 mb-8">
                                ...
                                <span className="text-lg font-bold text-slate-300 line-through">${(deal.price * 1.25).toFixed(0)}</span>
                                            ...
                                            <button 
                                              ...
                                              className={`flex items-center gap-2 text-xs font-bold uppercase tracking-widest transition-all ${addedItems[deal.id] ? 'text-emerald-500' : 'text-blue-600 group-hover:gap-4'}`}
                                            >
                                              {addedItems[deal.id] ? (
                                                <><Check size={16} /> Added</>
                                              ) : (

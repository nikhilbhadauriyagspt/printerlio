...
import { ShoppingCart, Heart, Plus, Zap, ChevronRight, Eye, Building2 } from "lucide-react";
import { Link } from "react-router-dom";
import { useCart } from "../context/CartContext";
import { motion } from 'framer-motion';
import { cn } from '../lib/utils';

export default function CategorySlider({ products = [], loading = false }) {
  ...
  return (
    <section className="bg-white py-16 md:py-24 w-full font-poppins border-t border-[#E8E6E1]">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div className="max-w-xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-50 text-black rounded-lg text-[11px] font-bold mb-3 uppercase tracking-widest">
               ...
               <span>Enterprise Choice</span>
            </div>
            <h2 className="text-3xl md:text-4xl font-bold text-[#1A1A1A]  leading-none mb-3">
              Professional <span className="text-black">Printers</span>
            </h2>
            <p className="text-gray-500 text-[14px] font-medium leading-relaxed">
              Industrial-grade printing solutions engineered for high-volume productivity.
            </p>
          </div>
          <Link 
            to="/shop" 
          ...
          >
            View Catalog <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform" />
          </Link>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 md:gap-5">
          {loading ? (
             Array.from({ length: 6 }).map((_, i) => (
               <div key={i} className="aspect-[4/5] rounded-3xl bg-[#F8F8F8] animate-pulse border border-[#E8E6E1]" />
             ))
          ) : (
                ...
                <div className="flex flex-col flex-1 px-1">
                  ...
                  <div className="flex items-center justify-between mt-auto">
                    ...
                    <button 
                      ...
                      className="h-9 w-9 rounded-xl bg-[#1A1A1A] text-white flex items-center justify-center hover:bg-black transition-all group/cart active:scale-90"
                    >

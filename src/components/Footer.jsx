...
export default function Footer() {
  ...
  return (
    <footer className="w-full bg-[#fbbf24] text-[#001e3c] pt-12 pb-6 font-['Poppins']">
      <div className="max-w-[1820px] mx-auto px-4 md:px-10">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 pb-10 border-b border-[#001e3c]/10">
          <div className="max-w-[400px]">
            ...
            <p className="text-[#001e3c]/80 text-[14px] leading-relaxed">
          ...
          <div className="w-full lg:max-w-[500px]">
            <h4 className="text-[12px] font-bold uppercase tracking-widest mb-3">Stay Informed</h4>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-12 gap-8 py-10">
          <div className="lg:col-span-3">
            <h4 className="text-[13px] font-black uppercase tracking-widest mb-8 border-b-2 border-[#001e3c] inline-block pb-1">Navigation</h4>
            <ul className="space-y-4">
              {navLinks.map((link) => (
                <li key={link.name}>
                  <Link to={link.path} className="text-[#001e3c]/70 hover:text-[#001e3c] hover:font-bold text-[14px] transition-all">{link.name}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="lg:col-span-3">
            <h4 className="text-[13px] font-black uppercase tracking-widest mb-8 border-b-2 border-[#001e3c] inline-block pb-1">Top Collections</h4>
            <ul className="space-y-4">
              {categories.map((cat) => (
                <li key={cat.id}>
                  <Link to={`/shop?category=${cat.slug}`} className="text-[#001e3c]/70 hover:text-[#001e3c] hover:font-bold text-[14px] transition-all">{cat.name}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="lg:col-span-3">
            <h4 className="text-[13px] font-black uppercase tracking-widest mb-8 border-b-2 border-[#001e3c] inline-block pb-1">Policies</h4>
            <ul className="space-y-4">
              {legalLinks.map((link) => (
                <li key={link.name}>
                  <Link to={link.path} className="text-[#001e3c]/70 hover:text-[#001e3c] hover:font-bold text-[14px] transition-all">{link.name}</Link>
          ...
          <div className="lg:col-span-3">
            <h4 className="text-[13px] font-black uppercase tracking-widest mb-8 border-b-2 border-[#001e3c] inline-block pb-1">Reach Us</h4>
            <div className="space-y-6">
              <div className="flex items-center gap-3">
                <Mail size={18} className="text-[#001e3c]" />
                <a href="mailto:info@printerlio.shop" className="text-[14px] font-bold border-b border-[#001e3c]/20">info@printerlio.shop</a>
              </div>
        ...
        <div className="pt-10 border-t border-[#001e3c]/10 flex flex-col md:flex-row justify-between items-center gap-8">
          <p className="text-[13px] font-medium text-[#001e3c]/60">
            © {new Date().getFullYear()} Printer Lio. Empowering your output.
          </p>

          <div className="flex items-center gap-10">
            <div className="flex items-center gap-2">
              <ShieldCheck size={20} className="text-[#001e3c]" />
              <span className="text-[11px] font-black uppercase tracking-widest">SSL Encrypted</span>
            </div>
        ...
        <div className="mt-5 py-4 bg-[#001e3c]/5 rounded-xl">
           <p className="text-[10px] font-bold text-[#001e3c]/40 text-center uppercase tracking-[0.25em] leading-relaxed px-4">
